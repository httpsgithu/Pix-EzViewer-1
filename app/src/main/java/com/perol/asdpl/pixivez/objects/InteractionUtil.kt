...
object InteractionUtil {
    ...
    private fun need_restrict(item: Illust) = (PxEZApp.R18Private && item.x_restrict == 1)
    private fun x_restrict(item: Illust): String = x_restrict(need_restrict(item))
    private fun x_restrict(restrict: Boolean): String {
        return if (restrict) {
            "private"
        }
        else {
    ...
    fun toDetailString(it: Illust, caption: Boolean = true) =
        "id:${it.id} " + (if (caption) "caption:${Html.fromHtml(it.caption)}" else "") +
            ...
            "AI: ${AIType.values()[it.illust_ai_type]}|${it.illust_book_style} tools:${it.tools}\n" +
            "type:${it.type} page_count:${it.page_count}\n" +
            "visible:${it.visible} is_muted:${it.is_muted}|${it.comment_access_control}\n" +
